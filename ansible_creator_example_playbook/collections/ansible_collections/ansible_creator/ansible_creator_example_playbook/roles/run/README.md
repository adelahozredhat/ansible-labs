Ansible_creator.Ansible_creator_example_playbook Run Role
========================
...
- name: Execute tasks on servers
  hosts: servers
  roles:
    - role: ansible_creator.ansible_creator_example_playbook.run
      run_x: 42
```
...
```yaml
- name: Initialize the run role from ansible_creator.ansible_creator_example_playbook
  hosts: servers
  gather_facts: false
  tasks:
    - name: Trigger invocation of run role
      ansible.builtin.include_role:
        name: ansible_creator.ansible_creator_example_playbook.run
      vars:
        run_x: 42
```
